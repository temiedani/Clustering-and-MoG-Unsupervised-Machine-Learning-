...
plot_filename = os.path.join(
    os.getcwd(), 'figures', 'dataset_phoneme_{}.png'.format(p_id))
...
for t in range(n_iter):
    ...
    for i in range(k):
        ...
        res_1 = np.squeeze(
            np.matmul(X_minus_mu, np.expand_dims(Z[:, i], axis=1)))/np.sum(Z[:, i])
        s[i, :, :] = np.diag(res_1)
        p[i] = np.mean(Z[:, i])
    ax1.clear()
    # plot the samples of the dataset, belonging to the chosen phoneme (f1 & f2, phoneme 1 or 2)
...
plot_filename = os.path.join(
    os.getcwd(), 'figures', 'GMM_phoneme_{}_k_{}.png'.format(p_id, k))
plt.savefig(plot_filename)
