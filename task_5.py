...
plot_filename = os.path.join(
    os.getcwd(), 'figures', 'dataset_3D_phoneme_{}.png'.format(p_id))
...
for t in range(n_iter):
    ...
    for i in range(k):
        ...
        term_2 = np.repeat(np.expand_dims(
            Z[:, i], axis=1), D, axis=1) * term_1.transpose()
        s[i, :, :] = np.matmul(term_1, term_2)/np.sum(Z[:, i])
        #########################################
        # Suggest ways of overcoming the singularity

        #########################################
        # Suggest ways of overcoming singularity
        s[i, :, :] = np.identity(D) * 0.01 + s[i, :, :]
        # this can be also written as below:
        # s[i, :, :] += np.eye(s.shape[1]) * 0.01
        # Method 2 to avoid singularity resetting the mean and variance when singularity occurs
        '''
